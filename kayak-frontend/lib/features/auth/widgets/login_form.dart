...
class _LoginFormState extends ConsumerState<LoginForm> {
  ...
  Future<void> _submitForm() async {
    print('[LoginFlow] _submitForm() called');
    // 验证表单
    final emailError = Validators.validateEmail(_emailController.text.trim());
    final passwordError = Validators.validatePassword(_passwordController.text);

    if (emailError != null) {
      print('[LoginFlow] Validation failed: emailError=$emailError');
      ref.read(emailValidationProvider.notifier).state = emailError;
      return;
    }
    if (passwordError != null) {
      print('[LoginFlow] Validation failed: passwordError=$passwordError');
      ref.read(passwordValidationProvider.notifier).state = passwordError;
      return;
    }

    print('[LoginFlow] Validation passed, calling authStateNotifier.login()');
    // 清除错误
    ...
    try {
      ...
      print('[LoginFlow] login() returned success=$success');

      if (success) {
        print('[LoginFlow] Setting login state to success');
        ref.read(loginProvider.notifier).setSuccess();
      } else {
        final authState = ref.read(authStateProvider);
        final errorMsg = authState.error;
        print('[LoginFlow] login() failed, state error=$errorMsg');
        ref.read(loginProvider.notifier).setError(
          _mapErrorToLoginErrorType(errorMsg),
        );
      }
    } catch (e) {
      print('[LoginFlow] login() threw exception: $e');
      ref.read(loginProvider.notifier).setError(
